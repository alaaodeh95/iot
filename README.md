...
- 🔒 **Comprehensive Security**: JWT authentication, API keys, request signing, encryption support
...
### 1. Security Setup (First Time Only)
...
This creates:
- `.env` file with secure API keys and JWT secrets
- SSL certificates in `./certs/` (optional)
...
- **API Keys**: Required for system endpoints and UI access
- **JWT Tokens**: Device-specific authentication with expiration
- **Device Keys**: Unique credentials per sensor device
- **Gateway Authentication**: Separate authentication for gateway service
...
### Testing Security
```bash
# Run security tests (after starting the controller)
python test_security_simple.py
```

### Security Configuration
Security settings are managed in `.env` file (generated by `security_setup.py`):
- `IOT_API_KEY`: Main API key for system access
- `JWT_SECRET_KEY`: Secret for signing JWT tokens
- `DEVICE_KEYS`: Individual keys for each sensor device
- `SSL_ENABLED`: Enable/disable HTTPS
- `ENABLE_REQUEST_SIGNING`: Enable request signatures
